...
public class ViewPagerIndicator extends LinearLayout {
	private Paint mPaint;// 绘制长方形的画笔
	private Path mPath;// 用于绘制长方形的边
	private int mTriangleWidth;// 长方形的宽
	private int mTriangleHeight;// 长方形的高
	private Context mContext;
	private static final float RADIO_TRIANGLE_WIDTH = 1 / 6F;// 用于设置长方形的宽和tab底边的比例，用于屏幕适配
	/**
	 * 长方形底边的最大宽度
	 */
	private final int DIMENSION_TRIANGLE_WIDTH_MAX = (int) (getScreenWidth()/3*RADIO_TRIANGLE_WIDTH);
	private int mInitTranslationX;// 第一个长方形初始化的偏移位置
	private int mTranslationX;// 移动时候的长方形偏移位置
	private int mTabVisibleCount;// 可见tab的数量
	private static final int COUNT_DEFAULT_TAB = 2;// 默认可见tab为2个
	private List<String> mTitles;// 接收传递过来的title
	private static final int COLOR_TEXT_NORMAL = Color.parseColor("#2a2a2a");
	private static final int COLOR_TEXT_HIGHLIGHT = Color.parseColor("#19b3cb");
	...
	private void initRectangle() {
		...
		// 将长方形角度设置为30度
		mTriangleHeight = DensityUtil.dip2px(mContext,2);
		mPath = new Path();
		mPath.moveTo(0,0 );
