...
public abstract class BaseCommonAdapter<T> extends BaseAdapter {
    ...
    protected void setMultiItemTypeSupport(MultiItemTypeSupport<T> itemTypeSupport) {
        this.mMultiItemTypeSupport = itemTypeSupport;
    }

    public BaseCommonAdapter(Context context, List<T> data, MultiItemTypeSupport<T> mMultiItemTypeSupport) {
     ...
     * your needs
     */
    protected ViewHolder getViewHolderManager(int position, View convertView, ViewGroup parent) {
        if (mMultiItemTypeSupport != null) {
            return ViewHolder.getViewHolder(context, convertView, parent, mMultiItemTypeSupport.getLayoutItemRes
                    (position, data.get(position)), position);
        } else {
            return ViewHolder.getViewHolder(context, convertView, parent, layoutItemId, position);
        }
    }
    ...
    public void setNewList(List<T> datas) {
        this.data = datas == null ? new ArrayList<T>() : new ArrayList<T>(datas);
        notifyDataSetChanged();
    }
    ...
    public void removerElemByIndex(int index) {
        if (data == null || index >= data.size()) return;
        data.remove(index);
        notifyDataSetChanged();
    }
